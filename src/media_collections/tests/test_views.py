...
class CollectionDetailViewTests(TestCase):
    ...
    def test_collection_detail_uses_default_page_size(self):
        ...
        self.assertEqual(response.context["items_per_page"], 48)
        self.assertEqual(len(response.context["collection_items"]), 48)
    ...
    def test_collection_detail_accepts_page_size_choice(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["items_per_page"], 24)
        self.assertEqual(response.context["collection_items"].number, 2)
        self.assertEqual(len(response.context["collection_items"]), 24)
