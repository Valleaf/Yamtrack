"""Fix country distribution and world map rendering."""
...
# ── 1. statistics.py — fix get_country_distribution to use media.item.country
#       and resolve ISO codes to names
print("\n[1] statistics.py — fix country field path + ISO name lookup")
...
_ISO_TO_NAME = {
    ...
    "KE":"Kenya","KG":"Kyrgyzstan","KH":"Cambodia","KI":"Kiribati","KM":"Comoros","KP":"North Korea",
    ...
    "UZ":"Uzbekistan","VA":"Vatican City","VE":"Venezuela","VN":"Vietnam","VU":"Vanuatu",
    "WS":"Samoa","YE":"Yemen","ZA":"South Africa","ZM":"Zambia","ZW":"Zimbabwe",
}

'''

stats_py = APP / "statistics.py"
stats_text = stats_py.read_text(encoding="utf-8")

if "_ISO_TO_NAME" not in stats_text:
    # Insert before get_country_distribution
    stats_text = stats_text.replace(
        "def get_country_distribution(user_media):",
        ISO_MAP + "def get_country_distribution(user_media):",
    )
    print("  OK   statistics.py — _ISO_TO_NAME map added")
else:
    print("  SKIP statistics.py — _ISO_TO_NAME already present")

OLD_FN = '''def get_country_distribution(user_media):
...
NEW_FN = '''def get_country_distribution(user_media):
    """Get media count by country name for each media type.

    Reads the ISO 3166-1 alpha-2 code from Item.country and resolves it to a
    human-readable country name for display in the statistics page.
    ...
    for media_type, media_list in user_media.items():
        ...
        for media in media_list.select_related("item"):
            # country is on Item, not on the media model itself
...
if OLD_FN in stats_text:
    stats_text = stats_text.replace(OLD_FN, NEW_FN)
    print("  OK   statistics.py — get_country_distribution fixed")
else:
    print("  SKIP statistics.py — get_country_distribution pattern not found (check manually)")

stats_py.write_text(stats_text, encoding="utf-8")

# ── 2. world-map-stats.js — replace bar chart with choropleth via Chart.js geo
print("\n[2] world-map-stats.js — replace with choropleth map")

JS = r"""// World map statistics visualization using Chart.js + chartjs-chart-geo
(function () {
  'use strict';
...
js_path = SRC / "static" / "js" / "world-map-stats.js"
js_path.write_text(JS, encoding="utf-8")
print("  OK   world-map-stats.js — choropleth map written")

print("\n✅ Done. Now:")
print("   git add src/app/statistics.py src/static/js/world-map-stats.js")
print("   git commit -m 'fix: world map choropleth + correct country field path'")
